...
def is_event_host():
    ...
    async def predicate(interaction: discord.Interaction) -> bool:
        ...
        allowed_role_names = {"Host", "Head Host 🍵🎮"}
        allowed_role_ids = {1378338515791904808, 1371003310223654974}

        user_roles = interaction.user.roles
        user_role_names = {role.name for role in user_roles}
        user_role_ids = {role.id for role in user_roles}

        if user_role_names & allowed_role_names or user_role_ids & allowed_role_ids:
            return True
