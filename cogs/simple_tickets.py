...
class SimpleTicketControls(View):
    ...
    async def claim_ticket(self, interaction: discord.Interaction, button: Button):
        """Claim or transfer ticket - unlimited times with cooldown"""
        ...
        # Update button to show current claimer
        button.label = f"✅ Claimed by {interaction.user.display_name}"
        button.style = discord.ButtonStyle.success
        button.emoji = "✅"
        
        # Change channel name to show who claimed it
        try:
            new_name = f"✅{interaction.user.display_name.lower()}-ticket"
            await interaction.channel.edit(name=new_name)
        except:
            pass
        
        # Send claim/transfer message
        if old_claimed_by and old_claimed_by != interaction.user:
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: Button):
        """Close the ticket - only for staff"""
        if not self._is_staff(interaction.user):
            await interaction.response.send_message(
...
class SimpleTicketButton(View):
    ...
    async def create_ticket(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            ticket_embed.set_footer(text="🎫 Simple Ticket System")
...
class SimpleTickets(commands.Cog):
    ...
    async def ticket_panel(self, interaction: discord.Interaction):
        embed = discord.Embed(
            ...
            description="**Need help?** Click the button below to create a support ticket.\n\n"
                       "**How it works:**\n"
                       "• Click the button to create a private channel\n"
                       "• Describe your issue in the ticket channel\n"
                       "• Staff will be notified and help you\n"
        ...
        embed.add_field(
            ...
            value="• Have all relevant information ready\n"
    ...
    @app_commands.command(name="close", description="🔐 Close a ticket (Staff only)")
    async def close_ticket(self, interaction: discord.Interaction):
        """Close the current ticket channel - restricted to the 4 staff roles"""
        # Check if user is one of the 4 specific staff roles
        if not self._is_staff(interaction.user):
            await interaction.response.send_message(
                "❌ Only **forgotten one**, **overseer**, **lead moderator**, or **moderator** can use this command!", 
                ephemeral=True
            )
            return
        ...
        # Find the ticket creator and remove from tracking
        creator_id = None
    ...
    def _is_staff(self, user) -> bool:
        """Check if user is one of the 4 authorized staff roles"""
        if user.guild_permissions.administrator:
            return True
        ...
        # Check the 4 specific staff roles
        user_roles = [role.name.lower().strip() for role in user.roles]
