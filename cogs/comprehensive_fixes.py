...
class ComprehensiveFixes(commands.Cog):
    ...
    @app_commands.command(name="lockchannel", description="🔒 Lock channel - only staff can message")
    async def lock_channel(self, interaction: discord.Interaction, channel: discord.TextChannel = None):
        """Lock a channel so only the 4 staff roles can message"""
        
        # Check if user is one of the 4 staff roles
        is_authorized = (
            interaction.user.guild_permissions.administrator or
            has_special_permissions(interaction.user) or
        ...
        if not is_authorized:
            embed = discord.Embed(
                title="❌ Access Denied",
                description="Only lead moderators, moderators, overseers, and forgotten ones can lock channels.",
                color=0xff0000
            )
        ...
        try:
            ...
            # Update permissions to deny send_messages for @everyone
            await target_channel.set_permissions(everyone_role, send_messages=False, reason=f"Channel locked by {interaction.user}")
            
            # Ensure staff roles can still message
            ...
            for role in interaction.guild.roles:
                if role.name.lower() in staff_roles or role.permissions.administrator:
                    await target_channel.set_permissions(role, send_messages=True, reason="Staff role - bypass channel lock")
            
            embed = discord.Embed(
                title="� Channel Locked",
                description=f"**Channel:** {target_channel.mention}\n**Locked by:** {interaction.user.mention}\n\n**Status:** Only staff can now send messages in this channel.",
                color=0xff0000,
                timestamp=datetime.now()
            )
            embed.add_field(
            ...
            embed.add_field(
                name="� To Unlock",
                value="Use `/unlockchannel` command",
                inline=True
            )
    ...
    @app_commands.command(name="unlockchannel", description="� Unlock channel - restore normal permissions")
    async def unlock_channel(self, interaction: discord.Interaction, channel: discord.TextChannel = None):
        """Unlock a channel to restore normal messaging permissions"""
        ...
        if not is_authorized:
            embed = discord.Embed(
                title="❌ Access Denied",
                description="Only lead moderators, moderators, overseers, and forgotten ones can unlock channels.",
                color=0xff0000
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        try:
            ...
            # Restore permissions to allow send_messages for @everyone
            await target_channel.set_permissions(everyone_role, send_messages=None, reason=f"Channel unlocked by {interaction.user}")
            
            embed = discord.Embed(
                title="� Channel Unlocked",
                description=f"**Channel:** {target_channel.mention}\n**Unlocked by:** {interaction.user.mention}\n\n**Status:** Normal messaging permissions have been restored.",
                color=0x00ff00,
                timestamp=datetime.now()
            )
            embed.add_field(
                name="✅ Restored Access",
                value="All members can now send messages in this channel again.",
                inline=False
            )
    ...
    def calculate_success_rate(self, successful_works: int, tier: str) -> float:
        """Calculate success rate based on experience and tier difficulty"""
        base_rates = {
            "entry": 0.85,
            "junior": 0.80,
            "mid": 0.75,
        ...
        base_rate = base_rates.get(tier, 0.80)
    ...
    def get_next_tier(self, current_tier: str) -> str:
        ...
        tiers = ["entry", "junior", "mid", "senior", "executive", "legendary"]
        try:
