    ...
    async def removeallcookies(self, interaction: discord.Interaction):
        ...
        class ConfirmView(discord.ui.View):
            ...
            async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
                try:
                    # Get current total cookies before removal
                    all_users = db.get_all_user_data()
                    total_cookies_before = sum(user_data.get('cookies', 0) for user_data in all_users)
                    
                    result = db.remove_all_cookies_admin(interaction.user.id)
                    
                    if result["success"]:
                        embed = discord.Embed(
                            title="🗑️ **Mass Cookie Removal Complete**",
                            description=f"Successfully removed **{total_cookies_before:,}** total cookies from **{result['users_affected']}** users",
                            color=0xff0000,
                            timestamp=datetime.now()
                        )
                        embed.add_field(
                            name="📊 **Removal Statistics**", 
                            value=f"🍪 **Total Cookies Removed:** {total_cookies_before:,}\n👥 **Users Affected:** {result['users_affected']}\n📉 **Average per User:** {total_cookies_before // max(result['users_affected'], 1):,}",
                            inline=False
                        )
        ...
        # Get current total cookies to show in warning
        try:
            all_users = db.get_all_user_data()
            total_cookies = sum(user_data.get('cookies', 0) for user_data in all_users)
            user_count = len([user for user in all_users if user.get('cookies', 0) > 0])
        except:
            total_cookies = 0
            user_count = 0
        ...
        view = ConfirmView()
        embed = discord.Embed(
            title="⚠️ **DANGER: Mass Cookie Removal**",
            description=f"This will remove **{total_cookies:,}** total cookies from **{user_count}** users!\n\n**⚠️ This action cannot be undone!**",
            color=0xff0000,
            timestamp=datetime.now()
        )
        embed.add_field(
            name="📊 **Impact Overview**", 
            value=f"🍪 **Total Cookies to Remove:** {total_cookies:,}\n👥 **Users with Cookies:** {user_count}\n📉 **Average per User:** {total_cookies // max(user_count, 1):,}",
            inline=False
        )
        embed.add_field(
            name="🚨 **Consequences**", 
            value="• All users will have 0 cookies\n• All cookie milestone roles will be removed\n• Leaderboard positions will be reset\n• This affects the entire server economy",
            inline=False
        )
