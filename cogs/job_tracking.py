    ...
    async def work_status(self, interaction: discord.Interaction, user: discord.Member = None):
        ...
        try:
            ...
            if hours_since_work >= job_data['demotion_threshold']:
                status = "🔴 **CRITICAL - DEMOTION DUE**"
                status_color = 0xff0000
            elif hours_since_work >= job_data['warning_threshold']:
                status = "🟡 **WARNING - WORK NEEDED**"
            ...
            embed = discord.Embed(
                ...
                description=f"**User:** {target.mention}\n**Job Role:** {job_name.replace('_', ' ').title()}\n**Status:** {status}",
                color=status_color,
                timestamp=datetime.now()
            )
            ...
            embed.add_field(
                name="⏰ Activity Timer",
                value=f"**Hours since last work:** {hours_since_work:.1f}h\n**Time until demotion:** {time_until_demotion:.1f}h\n**Warning threshold:** {job_data['warning_threshold']}h\n**Demotion threshold:** {job_data['demotion_threshold']}h",
                inline=True
            )
            ...
            # Action needed
            if time_until_demotion <= 0:
                action_text = "🚨 **WORK IMMEDIATELY** - Demotion overdue!"
            elif time_until_demotion <= 2:
                action_text = f"⚡ **WORK NOW** - Only {time_until_demotion:.1f}h left!"
            elif time_until_demotion <= 6:
                action_text = f"⚠️ **Work soon** - {time_until_demotion:.1f}h remaining"
            else:
                action_text = f"✅ **Safe for now** - {time_until_demotion:.1f}h remaining"
            ...
            embed.add_field(
                name="💡 24-Hour Policy",
                value="• Work at least once every 24 hours\n• Use `/work` command to reset timer\n• Automatic demotion after 24h inactivity\n• Set reminders to stay active!",
                inline=False
            )
            
            embed.set_footer(text="💼 24-Hour Activity Tracking • Use /work to reset timer")
            embed.set_thumbnail(url=target.display_avatar.url)
            
            await interaction.response.send_message(embed=embed)
            
        except Exception as e:
            await interaction.response.send_message(f"❌ Error checking work status: {str(e)}", ephemeral=True)
